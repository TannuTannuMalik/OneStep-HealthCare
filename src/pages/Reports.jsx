...
export default function Reports() {
  ...
  const loadReports = async () => {
    ...
    try {
      ...
      if (res.data.ok) {
        ...
        setReports(data);
        // check dispensing status for all reports with blockchain
        data.forEach((r) => {
          if (r.blockchainTx) checkDispensing(r.id);
        });
      } else {
  ...
  const checkDispensing = async (reportId) => {
    try {
      const res = await api.get(`/pharmacy/prescription/${reportId}`);
      if (res.data.ok) {
        setDispensingStatus((prev) => ({
          ...prev,
          [reportId]: {
            isDispensed: res.data.blockchain?.isDispensed || false,
            dispensedAt: res.data.blockchain?.dispensedAt || null,
            isValid: res.data.blockchain?.isValid || false,
          },
        }));
      }
    } catch {
      // prescription not on chain yet — ignore silently
    }
  };
  ...
  const verifyReport = async (reportId) => {
    ...
    try {
      ...
      setVerifyResults((prev) => ({
        ...prev,
        [reportId]: {
          success: !tampered, tampered, dbMatch, chainMatch, blockchainTx,
          message: tampered
      ...
      setVerifyResults((prev) => ({
        ...prev,
        [reportId]: { success: false, message: e.response?.data?.error || e.message || "Verification failed" },
      }));
    } finally {
  ...
  const verifyPrescription = async (reportId) => {
    ...
    try {
      ...
      setPrescriptionResults((prev) => ({
        ...prev,
        [reportId]: { success: false, message: e.response?.data?.error || e.message || "Prescription verification failed" },
      }));
    } finally {
  ...
  return (
    <>
      ...
      <div style={styles.container}>
        <div style={styles.topRow}>
          <div>
            ...
            <p style={styles.subtitle}>
              Reports become available after the appointment is completed and the doctor generates the PDF.
            </p>
          </div>
          <button
            style={{ ...styles.btnOutline, opacity: loading ? 0.7 : 1, cursor: loading ? "not-allowed" : "pointer" }}
            onClick={loadReports}
            disabled={loading}
          >
        ...
        {errMsg && (
          <div style={styles.errorBox}><strong>Error:</strong> {errMsg}</div>
        )}
        ...
        {loading ? (
          ...
          reports.map((r) => {
            ...
            const dispStatus = dispensingStatus[r.id];
            ...
            return (
              <div key={r.id} style={styles.card}>
                <div style={styles.cardHeader}>
                  <div>
                    ...
                    <div style={styles.metaText}>
                      <strong>Doctor:</strong> {r.doctorName || "—"} {r.specialty ? `• ${r.specialty}` : ""}
                    </div>
                    <div style={styles.metaText}>
                    ...
                    {hasBlockchain && (
                      <div style={styles.txText}>
                        ⛓️ <strong>Blockchain Tx:</strong> {r.blockchainTx.slice(0, 24)}...
                      </div>
                    )}
                  </div>
                  ...
                  <div style={styles.badgeColumn}>
                    ...
                    {/* DISPENSED BADGE */}
                    {dispStatus?.isDispensed && (
                      <span style={styles.dispensedBadge}>
                        💊 Dispensed
                      </span>
                    )}
                    {dispStatus && !dispStatus.isDispensed && dispStatus.isValid && (
                      <span style={styles.notDispensedBadge}>
                        ⏳ Not Dispensed
                      </span>
                    )}
                  </div>
                ...
                {/* Dispensed timestamp */}
                {dispStatus?.isDispensed && dispStatus.dispensedAt && (
                  <div style={styles.dispensedInfo}>
                    💊 Prescription dispensed by pharmacist on {formatDate(dispStatus.dispensedAt)}
                  </div>
                )}
                ...
                {result && (
                  <div style={{
                    ...
                    border: result.success ? "1px solid #9ad0ab" : "1px solid #efb0b0",
                    ...
                    {result.blockchainTx && (
                      <div style={{ marginTop: 6, fontSize: 13 }}>Tx: {result.blockchainTx}</div>
                    )}
                  </div>
                )}
                ...
                {prescriptionResult && (
                  <div style={{
                    ...styles.verifyBox,
                    background: prescriptionResult.success ? "#eef6ff" : "#fff1f1",
                    border: prescriptionResult.success ? "1px solid #9fc3f3" : "1px solid #efb0b0",
                    color: prescriptionResult.success ? "#1a3a6b" : "#a12a2a",
                  }}>
                    <div style={{ fontWeight: 700 }}>{prescriptionResult.message}</div>
                    {prescriptionResult.prescriptionHash && (
                      <div style={{ marginTop: 6, fontSize: 13 }}>Hash: {prescriptionResult.prescriptionHash}</div>
                    )}
                    ...
                    {prescriptionResult.timestamp && (
                      <div style={{ marginTop: 6, fontSize: 13 }}>Stored At: {formatDate(prescriptionResult.timestamp)}</div>
                    )}
                  </div>
                )}
                ...
                <div style={styles.actions}>
                  {hasPdf ? (
                    <>
                      <button type="button" style={styles.btn} onClick={() => viewReport(r.id)}>View PDF</button>
                      <button type="button" style={styles.btnOutline} onClick={() => downloadReport(r.id)}>Download PDF</button>
                      {hasBlockchain && (
                        <>
                          <button
                            type="button"
                            style={{ ...styles.btnVerify, opacity: verifying[r.id] ? 0.7 : 1, cursor: verifying[r.id] ? "not-allowed" : "pointer" }}
                          ...
                          <button
                            type="button"
                            style={{ ...styles.btnOutline, opacity: checkingPrescription[r.id] ? 0.7 : 1, cursor: checkingPrescription[r.id] ? "not-allowed" : "pointer" }}
                            onClick={() => verifyPrescription(r.id)}
                            disabled={checkingPrescription[r.id]}
                          >
                            {checkingPrescription[r.id] ? "Checking..." : "💊 Verify Prescription"}
                          </button>
                        </>
                      )}
                    </>
                  ) : (
                    <div style={{ opacity: 0.8 }}>PDF not uploaded yet. Ask the doctor to generate the report.</div>
                  )}
                </div>
              </div>
            );
...
const styles = {
  ...
  topRow: { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 20 },
  subtitle: { marginTop: 6, opacity: 0.8, lineHeight: 1.5 },
  card: { background: "#f9f9f9", padding: "20px", marginBottom: "20px", borderRadius: "12px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" },
  cardHeader: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" },
  badgeColumn: { display: "flex", flexDirection: "column", gap: 8, alignItems: "flex-end" },
  ...
  btnVerify: { padding: "10px 16px", background: "#1b7a3c", color: "white", border: "none", borderRadius: "6px", cursor: "pointer", fontWeight: 700 },
  verified: { fontSize: 12, fontWeight: 700, color: "#1b7a3c", background: "#dff7e6", padding: "6px 10px", borderRadius: 999 },
  pending: { fontSize: 12, fontWeight: 700, color: "#8a5b00", background: "#fff3cd", padding: "6px 10px", borderRadius: 999 },
  blockchain: { fontSize: 12, fontWeight: 700, color: "#1a3a6b", background: "#dce8ff", padding: "6px 10px", borderRadius: 999 },
  dispensedBadge: { fontSize: 12, fontWeight: 700, color: "#1d4ed8", background: "#dbeafe", padding: "6px 10px", borderRadius: 999, border: "1px solid rgba(37,99,235,0.2)" },
  notDispensedBadge: { fontSize: 12, fontWeight: 700, color: "#92400e", background: "#fef3c7", padding: "6px 10px", borderRadius: 999 },
  errorBox: { padding: 12, borderRadius: 10, background: "#ffe3e3", border: "1px solid rgba(156,42,42,0.3)", color: "#9c2a2a", marginBottom: 16, fontWeight: 700 },
  infoBox: { padding: 16, borderRadius: 10, background: "#f5f7fb", border: "1px solid #d9e2f2" },
  emptyBox: { padding: 16, borderRadius: 10, background: "#fff", border: "1px dashed #bbb" },
};